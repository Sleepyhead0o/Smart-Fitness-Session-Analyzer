...
def generate_fitness_data(
    ...
    for timestamp in range(number_of_windows):
        ...
        if scenario == "resting":
            ...
            activity = _clamp(
                rng.gauss(0.10 + 0.75 * decline, 0.05),
                0,
                1
            )
            skin_offset = rng.gauss(
                0.08 + 0.55 * decline,
                0.10
            )
            temperature_offset = rng.gauss(
                0.05 + 0.45 * decline,
                0.08
            )
        ...
        observation = {
            ...
            "heart_rate": int(
                round(
                    _clamp(
                        baseline_hr + hr_offset,
                        35,
                        205
                    )
                )
            ),
            "skin_response": _rounded(
                max(
                    0,
                    baseline_skin + skin_offset
                )
            ),
            "temperature": _rounded(
                _clamp(
                    baseline_temperature + temperature_offset,
                    25,
                    42
                )
            ),
            "activity_level": _rounded(
                _clamp(
                    activity,
                    0,
                    1
        ...
        if scenario == "poor_quality":
            observation["signal_quality"] = _rounded(
                rng.uniform(
                    0.05,
                    0.55
                )
